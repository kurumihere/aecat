...
static void cat_n_args(int fd, const char *name, const char *prog)
{
	...
	if (n < 0) cat_err(prog, name);
}

static void 
cat_args(int fd, const char *name, const char *prog, cat_opts *opts)
{
	static int l_num = 1;
...
static int parse_args(int c, char **av, cat_opts *opts)
{
	...
	for (i = 1; i < c; i++) {
		if (av[i][0] != '-' || av[i][1] == '\0') 
			continue;
		if (av[i][1] == '-') {
			if (strcmp(av[i], "--number-nonblank") == 0) opts->num_nonblank = true;
			else if (strcmp(av[i], "--number") == 0) opts->num_all = true;
			else if (strcmp(av[i], "--squeeze-blank") == 0) opts->squeeze = true;
			else return -1;
			continue;
		}
		for (j = 1; av[i][j]; j++) {
			switch (av[i][j]) {
				case 'b': opts->num_nonblank = true; break;
				case 'e': opts->show_ends = true; break;
				case 'n': opts->num_all = true; break;
	...
	if (opts->num_nonblank) opts->num_all = false;
	return 0;
}
...
main(int c, char **av)
{
	...
	use_flags = (opts.num_all || opts.num_nonblank || 
		     opts.show_ends || opts.squeeze || opts.show_tabs);
	for (i = 1; i < c; i++) {
		if (av[i][0] == '-' && av[i][1] != '\0') 
			continue;
		has_files = true;
